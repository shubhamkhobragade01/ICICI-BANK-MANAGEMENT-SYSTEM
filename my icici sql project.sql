...
#2.count the number of employees working in the loan department and show its opening dates and address.alter
...
join bank_details on account_type.Account_no = Account_no
join bank_details on department.Department_id = bank_details.Department_id
where account_type.Department_name =
group by account_type.Opening_Date, bank_details.Adress;

#3.find details department_name, address, branch code, dept_id, city of the account no 18190.alter

select account_type.Department_name,department_id, bank_details.Adress, bank_details.Branch_code, customer.city
from account_type
...
select department.Departrment_id,department_name, job_details, job_id from department
join job_details on department.Department_id = job_details.Department_id
where department.Department_name in ("loan","HR","admin");

#5.find the account_type .state account number whose are atm no 423705689
...
where customer .Atm_no = 423705689;

#6.create a view with that show address,branch name, department name, first name.phone no

create view details as 
select department.Department_name, bank_details,Adress, bank_details.Branch_name, employees.First_name, employees.phone_no
from department
join bank_details on department.Department_id = bank_details.Department_id;

select * from details;

#7.create view city, department name whsoe opening date is less than 24 may 04
